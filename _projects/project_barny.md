...
개념적 설계 – 논리적 설계 – 물리적 설계 순으로 진행하였으며, 최종적으로 Eclipse의 E-R 모델링 도구인 eXERD를 통해 설계를 완성하였습니다. Barny의 DB 내 테이블은 회원, 게시판, 장바구니를 포함하여 총 14개로 구성되어 있습니다.
...
선택지를 대표하는 패키지들은 배열로서 존재하며, 각각의 값은 0으로 시작합니다. 질문마다 사용자가 선택지를 선택하면, 해당 선택지에 대응되는 배열의 요소에는 1씩 더해지는 방식으로 구현하였습니다.

###### 테스트 종료 후의 결과 페이지

```javascript
function calResult() {
**_console_**.log(**_select_**);
var result = **_select_**.indexOf(**_Math_**.max(...**_select_**));
return result;
}

function setResult() {
let point = calResult();
const resultName = **_document_**.querySelector('.resultname');
resultName.innerHTML = **_infoList_**[point].name;

var resultImg = **_document_**.createElement('img');
const imgDiv = **_document_**.querySelector('#resultImg');
...
const resultDesc = **_document_**.querySelector('.resultDesc');
resultDesc.innerHTML = **_infoList_**[point].desc;
}
```
...
또한 각각의 이벤트는 사용자가 하루에 한 번만 참여하도록 설정하였습니다. event 테이블에는 해당 이벤트의 board_id 및 사용자의 member_idx, 그리고 사용자가 참여한 날짜인 participate_date가 insert됩니다. 따라서 룰렛 이벤트가 구현되면, js와 principal을 통해 로그인한 사용자의 participate_date와 현재 날짜를 비교하는 함수가 호출되어, 일치 시에는 참여를 제한하는 alert이 호출되고, 이벤트 실행 함수는 취소됩니다.
...
부분 삭제의 경우 각각의 게시물 앞에 체크박스를 표시한 뒤 선택된 게시물은 배열에 값을 저장하도록 하였습니다. 전체 삭제 버튼을 누를 경우에는 표시된 게시물 전체가 선택되도록 하였습니다.

선택된 게시물은 사용자에게 alert을 통해 재확인한 뒤 ajax를 통해 DB의 데이터가 삭제가 가능하도록 했습니다.
...
선택된 게시물은 ncontroller에서 for 반복문을 통해 삭제 명령이 적용되도록 구현하였습니다.

### 후기 및 보완점

우선 프로젝트를 진행하면서Spring Framework에 대한 전반적인 이해, 및 MVC 패턴, Restful한 방식에 대한 이해하고 활용할 수 있게 되었습니다.

또한 개인 프로젝트가 아닌 6명이 함께하는 협업이었던 것만큼 커뮤니케이션 능력 및 Git 활용 능력이 향상될 수 있었습니다.

다만 설계가 미흡하여 DB를 여러 번 수정했던 점이 아쉽습니다. 엔티티를 좀 더 세분화하여 상세하게 DB를 구성했어야 한다는 생각이 들었습니다.

또한 DB 공유가 제대로 되지 못해 차질이 생겼던 점, 일부 기능은 하드코딩으로 구현된 점이 아쉬웠습니다. 프론트단을 구현할 때 역시 태그 사용법이 미숙하여 Spring Framework의 'tiles' 혹은 jsp의 'include'를 사용하는 것이 아니라 공용으로 사용할 페이지들을 일일이 삽입하기도 했습니다.
